...
| Fifth round, the sandwich liked and the names-on-screen ruling | *"I like the spec for the friend match buzzers, it should say jump ball somewhere too right?"* | row 13 · every race screen names its moment, facing both ends in a friend match |
| Fifth round, the full dome family ordered and delivered | *"can I see it next to how the toss up online will look, the jump ball online, and the jump ball in CPU... I want to see the practice run ones too"* | seven mocks on the real surfaces: <https://claude.ai/code/artifact/15f0790d-46c6-49e4-a038-f9650c4dc1a2> |
| Fifth round, a shipped naming bug found by his flow question | his list's question marks led straight to it | game.js startColorCall told the TOSS-UP winner's opponent they "won the tip", the wrong race's name, live in the product; the 08-31 sweep had compounded it to "won the jump ball". FIXED: "won the toss-up." |
| Sixth round, three build specs off the family | *"it should say something along the line of this is the same for toss up and jump ball, but only in the toss up practice run"* · *"The visuals of the toss up and the jump ball need to match (I like the toss up version more)"* · *"Yes we need to fix the coach overlapping the buzzer"* | row 13's build spec · walkthrough doc § RULED 08-31 sixth round |
| Sixth round, his online question answered from code | *"what happens to the other player [if] one player gets to the toss up screen before the other in the online mode?"* | the How-it-works card is the waiting room: I'M READY becomes "Waiting for your friend…" and the host fires the shared countdown only when both readies are in (tuMarkReady) |
...
- **2026-09-04 · THE OPENING REBUILT ON THE REAL COURT (rows 217-222).**
  ...
  turn said once (223). Fleet: 45 gates green. **Same night, his answer**: *"the scoreboard should stay throughout the drop, but the skip button shouldn't sit on top of it, put it just below"*: built as `body.opening` (the rig stays, the banner, the note bar, the strip, the music tab and the view reset wait until the camera is home; both Skips just below the rig, a new one for the drop). cine-check 38 checks, five sabotages; fleet 45 green again. Row 222 closed with it.
