...
while (Date.now() - t0 < 120000) {
  await sleep(600);
  const st = await p.evaluate(() => { const T = BKFLOW.T(); const s = BK.state(); return {T: T && T.phase, st: s.phase, side: T && T.side, card: document.getElementById('qveil').classList.contains('on'), shoot: !!(document.getElementById('flShoot') && !document.getElementById('flShoot').disabled), end: !!document.getElementById('flEnd'), car: !!document.getElementById('mbCar')}; });
