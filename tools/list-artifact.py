...
def shipped_ledger():
    """every changelog heading in BUILD.md: the DONE half of the picture.
    Aaron, 08-24: "I need to visually see a complete picture and be able to
    SEE what's done and what needs to be done." The changelog stays the ONE
    home of shipped work; this renders its headings, so it cannot drift."""
    txt = open('BUILD.md').read()
    lines = txt.split('\n')
    out = []
    i = 0
    while i < len(lines):
        m = re.match(r'- \*\*(20\d\d-\d\d-\d\d[^·—]*)[·—]?\s*(.*)', lines[i])
        if m:
            date = m.group(1).strip().rstrip('*').strip()
            title = m.group(2)
            j = i
            while '**' not in title and j + 1 < len(lines):
                j += 1
                title += ' ' + lines[j].strip()
...
ledger = shipped_ledger()
led_rows = ''.join(
    f'<div class="led"><span class="ldate">{H.escape(d)}</span>'
    f'<span class="ltitle">{H.escape(t)}</span></div>' for d, t in ledger)
sections.append(
    f'<section><div class="lh"><span class="ln done">✓</span><h2>SHIPPED · the ledger</h2>'
    f'<span class="count">{len(ledger)} entries</span></div>'
    f'<p class="lsub">Everything that ever closed, straight from BUILD.md\'s changelog, '
    f'newest first. A row leaves the lists above only by landing here (or on SCRAPPED). '
    f'This is the DONE half of the picture.</p>{led_rows}</section>')
...
.ln.done{{background:#3a7d44;color:#eafbe7}}
.led{{display:flex;gap:12px;align-items:baseline;padding:7px 10px;border-bottom:1px solid rgba(255,176,58,.08)}}
.ldate{{flex:0 0 150px;font-family:var(--mono);font-size:11px;color:var(--dim);white-space:nowrap}}
.ltitle{{font-size:13.5px;color:var(--faint)}}
...
<header>
  ...
  <p class="lede">Every row of all six lists, numbers and notes included, and the
  <b>shipped ledger</b> at the bottom: the complete picture, open and done in one place,
  nothing behind the scenes. <b>List 1 is in ruled order</b> (Aaron, 08-24): the rank on
  the left is the plan to the twenty, the <b>#number</b> is the row's permanent name and
  never changes when it moves. Generated from TODO.md and BUILD.md's changelog, so if
  this page and the files ever disagree, the files win and this page gets republished.</p>
</header>
{''.join(sections)}
