...
| 102 | — | The action dock is cut in half on a phone: MOVE is entirely off screen | me | open | measured 08-22 at 390x844: the slim strip needs 493px and gets 298px, so PASS is clipped and MOVE never renders on screen. Desktop shows 493 of 493, which is why every check stayed green. Row 22's taller court is what forced the collapse to happen every turn instead of rarely. Shape is Aaron's call: shorter content on one line, or stacked rows and the court gives back the band. Not a re-open of V0 row 19, which shipped correctly: this is row 22 breaking it three days later |
