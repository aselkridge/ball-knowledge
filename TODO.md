...
| 191 | — | **Settings polish batch**: the broken Run-the-Coach-again control (serif fallback face, text overflowing a tiny circle), frame-inside-frame on the court picker, sub-label contrast at 3.76:1, the cool-grey slider track, the arrow-keys caption on touch phones | me | open | audit 08-25, refs group; the Start-over control is a plain visible break and leads the batch. Aaron 08-25, twice in the board replies: *"I have some concerns about the settings screen"* and the circle under the Start over text is *"one of the problems I have with this screen"*; his concerns are not yet named, so this batch opens by asking him to walk the screen **08-26, found and FIXED before he walked it**: the screen was a centred column taller than the window, and a centred column that overflows loses its top, so its own title sat 236px above the glass on a 390x844 phone and no amount of scrolling reached it (also 189px at 414x896, 304px at 390x664, 186px on desktop). One line in `.screen`, `justify-content:safe center`, and every screen was swept for the same clip: settings was the only one. What ALSO already landed on this screen and does not need re-reporting: sub-labels lifted to `--ink-mid`, the dead volume channel dimmed, the reserved band under the floating chrome. What is still HIS to name: the concerns he mentioned twice, plus anything the walk turns up **HIS WALK, 08-27.** (a) Themes: *"I think we are good"*, one maybe: the crate caption should read swipe on a phone and arrow-keys on a desktop, *"idk it's fine either way"*. (b) THE LIT LAW IS VIOLATED HERE and he called it: *"your one thing lit per page I think is violated here maybe? From the theme block and the selected theme and other things?"* Counted on the real build at 390px: SEVEN lit things, the biggest being the theme block itself at 84,317px squared wearing a border AND a glow, which is a CONTAINER wearing the ring that belongs to the one bright action. Also lit: the Start over ring, the ON toggle, three court rings inside the theme minis. The screen has no primary action at all, so the honest question for the options round is what deserves the one light here, if anything, and selection needs its own treatment that is not the lit treatment. (c) The Start over control gets an OPTION BOARD, his ask: *"needs redoing, send me some options love to see them visually"*. (d) The New main menu switch is SCRAPPED, see row 200 |
| 199 | — | **The red gates: 39 harnesses, 4 of them guarding laws the game no longer has** | me | open | found 08-26 by `tools/gates.mjs`, written the same hour after the music gate turned up red and had been red for a day: the end-of-block gate list was picked by hand, so a ruling that retired an old law left its gate rotting unseen. The runner now runs all 39 and re-runs a red one alone before believing it. FIXED the same turn: music (the badge law changed under 189, and the pause fold had quietly closed the only door to the player), lit (amended to match), feedback (`eras` really had vanished from every tester's bug report). STILL RED, each needing a read of the current law before its gate is rewritten: **turn-economy** (asserts the pre-Method-B free step; Method B owns full-court 5-on-5 now, so the gate tests a world that only exists in half-court and net games), **menu2** (expects 7 live drills, finds 11), **tape** (expects 14 source_register sites, finds 17), **verified-gate** (expects the verified gate to ship OFF; it does not). The last three are counts and switches that look like the gate is behind the build, but none of the three has been confirmed yet: confirm, then fix the gate or the game, and say which |
| 201 | — | Sweep the CSS the retired numbered menu leaves behind | me | open | filed 08-27 with row 200. The markup and the scaffolding go in 200; the style rules that only ever dressed `#screen-title` need a careful pass, because classes like `.ctrlbtn` are shared with live controls (the settings Start over button is one) and cannot be swept by name alone |
| 202 | — | **Does Send feedback belong in the pause menu, in settings, or both?** | me | open | his question 08-27: *"since send feedback is in settings then we don't need it as an option in pause menu if they can get to it in settings right?"* Answer with evidence before removing anything: a report sent from a live game carries the score, phase, possession, target and era, and a report sent from a menu carries none of that, so the two buttons do not send the same thing. Check first whether settings is even reachable mid-game without abandoning the run |
| 192 | — | **Setup flow polish batch**: the one ask gets the light (league), label-vs-pill collision (STREET LEGENDS), one primary-button spec across the five screens, focused-field halo only (names), Reshuffle demoted to ghost (squad), the era header saying one thing once, the 4 QTRS card and floating blurb (rules), the stats hint contrast (squad) | me | open | audit 08-25, setup-a and setup-b groups, verified per finding. 08-26: two of this batch's items landed early inside 189's lit sweep (Reshuffle to ghost; the era screen's second primary demoted to a selected toggle); do not redo them |
