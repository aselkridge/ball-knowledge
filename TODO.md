...
| 108 | — | **Back from the rulebook mid-match strands the game frozen with no menu** | me | open | Measured 08-24 (`tools/pause-paths-check.mjs`, B road): pause → How to play → Back lands on the game screen with the pause menu gone but the freeze still on, so the board looks live and neither clock ever ticks again until the player pauses and resumes by hand. The music tab's stop/play badge is stale on the same path (bb-switch never repainted). The Settings road already does this right, returning to the pause menu with the veil up: my recommendation is Back does the same, one line plus a bbPaint in `btnBack`'s howFromPause branch. Whose call the target behaviour is: Aaron, since resume-directly is also defensible |
| 109 | — | **A rulebook drill tapped mid-match destroys the running game silently, and the way back out is a black screen** | me | open | Measured 08-24 (D road): pause → How to play → any of the 11 live drill buttons boots the drill sandbox with NO confirm, the 7-4 match becomes 0-0, the CPU opponent is dismissed. Then End drill → Back leaves NO screen visible at all, just the arena backdrop (screenshot `design/shots/pause-paths/D-after-back.png`): endDrill's `show('how')` strips the game screen's `on`, and Back's howFromPause branch only removes classes, it never shows anything. By code read the online case is worse still: `startDrill` sets `net.on=false` without `leaveRoom()`, so the opponent is orphaned mid-room. Smallest honest fix is a confirm ("this ends your game") or hiding the drill buttons while a match is live; the black screen is a plain bug whatever is ruled. Whose call the design half is: Aaron |
## 2 · RESEARCH — active
