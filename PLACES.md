...
| 2026-08-23 | **ONE TAP OF THE COACH** (item 103, the coach). His speed dial, built properly from his reference image, beside one-tap-one-tip. Both anchored above the dock band after the first render put them through the SHOOT strip. Build: `node tools/coach3-shots.mjs` then `python3 tools/coach3-artifact.py` | https://claude.ai/code/artifact/e815d3c9-8100-471f-8322-5a38cf466466 |
| 2026-08-22 | **WHERE THE COACH STANDS** (item 103, the HUD). The coach as a button in the two places he named, bottom left opposite the music or in the pause menu beside the score, using the portrait the game already had rather than a drawn icon. Supersedes the top-right question-mark board. Build: `node tools/coach2-shots.mjs` then `python3 tools/coach2-artifact.py` | https://claude.ai/code/artifact/7264579f-52ca-4acf-b330-6d81d2eee3ca |
| 2026-08-22 | **MUSIC ON THE CLOCK** (item 103, the HUD). The in-game music button as a one-tap stop/play switch, with the boombox returning only on pause. Three glyph options, both states, and the measured problem it fixes: today one tap opens a 202x162 player on top of the floor with the clock running. Build: `node tools/music-shots.mjs` then `python3 tools/music-artifact.py` | https://claude.ai/code/artifact/0dc0e796-982b-480f-bbca-b65bd5f3a12d |
...
| 2026-08-25 | **THE DESIGN BIBLE RULINGS** (item 111). The eight decisions that turn the visual census and the 20-screen audit into law: what LIT means, the orange and ink ladders, radius, type and elevation scales, the radio, Sedgwick Ave, and the font shop with live samples and buy links. Build: assembled from `tools/visual-census.py --json` and the audit result; republish after his rulings land | https://claude.ai/code/artifact/1cf27566-ed1c-447b-9910-6bd0c9aece01 |
| 2026-08-26 | **THE DISPLAY VOICE** (F ruling shipped). Before/after for the Anton to Big Shoulders Black swap: seven guarded pairs, phone and desktop, Sedgwick's territory shown kept. Befores from a git worktree at the pre-change commit | https://claude.ai/code/artifact/ed6e0724-3013-4e06-a431-a528a83d3054 |
| 2026-08-26 | **THE LIT SWEEP, PART ONE** (row 189). Before/after for the first two lit-law surfaces: the league picker and the settings audio card, with render guards | https://claude.ai/code/artifact/ad4a42ed-8d56-411a-a31a-de3f9e31232c |
| 2026-08-26 | **THE LIT SWEEP, COMPLETE** (row 189 closed). Era, squad, streak and the music tab before/after, the standing gate, the play-layer hunt verdicts | https://claude.ai/code/artifact/c5932384-83f6-4855-bc8f-c76ead411d6b |
| 2026-08-26 | **THE RESERVED BAND** (row 190 closed). Floating chrome never sits on copy: the band, the chevron's scrim, the boombox that folds; before/after both viewports | https://claude.ai/code/artifact/f688fc0e-7613-4a69-9280-0e0e4c4bf269 |
| 2026-08-27 | **THE SETUP FLOW, STRAIGHTENED** (row 192 first pass). The league collision, the word floor, the era line, the three formats, the level blurb and the radius ladder, with before and afters, plus the two items that are his to rule | https://claude.ai/code/artifact/04f23ee3-439b-400e-9355-3a79345e8bef |
| 2026-08-28 | **The local relay and the two-peer harness.** `cd server && npm install`, then `PORT=8901 node index.js`, and drive it with `node tools/two-peer.mjs` (game server on :8899). Two headless phones join one real room over the real relay; `?server=ws://127.0.0.1:8901` is a parameter the build already reads, so nothing is patched to make it work. Health check: `curl http://127.0.0.1:8901/health`. First two-browser check in the repo, and the instrument row 128 is verified with | local only, no URL |
| 2026-08-28 | **THE ROAD TO THE TWENTY** (the ruled order, row 206 closed). Seven stops in order with one line each on why it sits there, what rides inside each stop and what closes with it, what moved and why, and the two caveats. This REPLACED the analysis board it was published from: that page argued the case in five waves and he said it was too confusing, which was fair, he had asked what to do and not why the list was wrong. Build: `python3 scratchpad/order-page.py`, which reads BUILD live out of TODO.md and FAILS if the page names a row that is not there or claims an order the file does not have. Republish after any re-order | https://claude.ai/code/artifact/52d2dbe2-6310-4ec2-9b26-993422f6d039 |
| 2026-08-29 | **THE COACH HIGHLIGHT BOARD.** Five ways the coach could mark the thing he is pointing at, each on the real screen in two subject shapes (the countdown glyph and a real button), motion frozen at the same instant so the only difference is the option. **RULED: option 1, the ring.** Rebuild: `node scratchpad/shoot-highlight-options.mjs` then `python3 scratchpad/build-hl-board.py` | https://claude.ai/code/artifact/7e0e501d-533d-4000-8ebf-182c09b88550 |
| 2026-08-28 | **THE TUNNEL INTRO, rough pass.** A working preview of the tunnel-to-tip-off cinematic he described: the push toward the light, the sky-to-court drop, center court close-up (two silhouettes, a ref), then the confirmed toss-up beats (countdown explained, both buzzers the instant the card appears, the question typing itself out) into a Game Starts card. Stand-in art only (CSS rings, no photo) — his ruling was source the art, build the movement. Skip works mid-sequence, reduced-motion completes. Build: `python3 scratchpad/tunnel-demo.py`, then verify with `verify-tunnel.mjs` and `verify-skip.mjs` before republishing | https://claude.ai/code/artifact/569949f9-525e-4ac3-8289-5a27167a35fe |
| 2026-08-28 | **THE SETUP ROAD** (his six, answered). The three repairs proved, the six forward controls cropped off the walked flow at one shared scale (five agree to within 1px of height, the league screen's is a 135x36 pill inside a card), the whole CPU road as a filmstrip with what each stop calls itself and how many things wear the accent, the rules page measured at 12 non-grey colours with LEGEND lit while unpicked, and the B19 spec written from his own Apple video set beside his own words. Carries two option LISTS awaiting his pick: the rules-page redesign (row 203) and the flow study (row 12). Build: `python3 scratchpad/pack.py` then `road-page.py` | https://claude.ai/code/artifact/71d015f7-378b-4d97-967a-a10ebba294f6 |
| 2026-08-27 | **THE MENU GOES QUIET**. Option A built (four lit down to two), the back arrow and the volume rail brought under the same ruling, and three ways to close the court card's double frame | https://claude.ai/code/artifact/fb3f6e3f-40f5-4c2b-8ddb-9db3cd3d9862 |
| 2026-08-27 | **THREE SHIPPED, ONE TO PICK**. The re-arm control in both its states, the theme block and the crate hint with before and afters, and the main menu's lighting as three renders on the real screen | https://claude.ai/code/artifact/dec60201-0e3b-4eb7-89f2-212c1682eeee |
| 2026-08-27 | **SIX ANSWERS AND A BUTTON** (his settings walk, answered). Four Start over options at real size with measurements and a recommendation, the lit-element count on settings and the main menu, the retired numbered menu before and after | https://claude.ai/code/artifact/540816e8-b37a-4a9d-8ac7-4e2e23981747 |
| 2026-08-26 | **THE SETTINGS WALK** (row 191's opener). The settings screen measured and pinned, seven items numbered so he can point at a number, the title-clip fix shown before and after at four sizes, and the two gate finds of the day | https://claude.ai/code/artifact/1c8362fd-bcf4-4183-b345-5a86d200b59d |
| 2026-08-26 | **THE OWNER'S MANUAL** (Aaron's private page, never shipped to players). How the game actually works: the detailed version, the very plain version, the lunch-and-learn skeleton. Re-measure and republish when the architecture changes | https://claude.ai/code/artifact/df355cee-3011-4c1b-83a6-7acfb043722b |
| 2026-08-24 | **THE WHOLE LIST.** Every row of TODO.md's six lists on one page, 102 rows, with BUILD in the ruled order to the twenty (his 08-24 ordering ruling). Generated FROM the file so it cannot drift: republish after any meaningful list change. Build: `python3 tools/list-artifact.py <out.html>` then publish to this URL | https://claude.ai/code/artifact/dab6fedc-5f69-4e17-9ca8-53853fa9e3a0 |
| 2026-08-24 | **THE TWO-CONTROL HUD** (item 103 shipped, plus fixes 108 and 109). Before/after of the shipped dock (pause symbol A, replay with grey/orange states, roomy inset, 64px desktop, the ⋯ tray retired), the rulebook-Back landing on the timeout again, and the mid-match rulebook withholding its drill launchers. Real screenshots both viewports; the befores shot from a git worktree at the pre-ship HEAD. Gates: `tools/hud-check.mjs` 24, `tools/pause-paths-check.mjs` 35, all sabotage-proved. One open question on the page: does the grey replay still read as a control at 30px? | https://claude.ai/code/artifact/a4ededa4-abfb-40ab-97fd-5d22c2a80bf3 |
